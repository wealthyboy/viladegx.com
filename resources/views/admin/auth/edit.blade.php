...
<div class="row">
   <div class="col-md-8 col-md-offset-2">
      ...
      <div class="card">
         <form   id="register"  method="POST" action="" aria-label="">
            {{ csrf_field() }}
                        ...
                        <input id="password"   type="password" class="form-control{{ $errors->has('password') ? ' is-invalid' : '' }}" name="password" >
                                    ...
                                    <div class="input-group">
                                        ...
                                        <div class="form-group label-floating">
                                            ...
                                            <input id="address"  required="true" type="text" class="form-control{{ $errors->has('address') ? ' is-invalid' : '' }}" value="{{null !== $user->address ?  $user->address->address : ''}}" name="address" required>
                                        </div>
                                    </div>
                                    ...
                                    <div class="input-group">
                                        ...
                                        <div class="form-group label-floating">
                                            ...
                                            <input id="city"  required="true" type="text" class="form-control{{ $errors->has('city') ? ' is-invalid' : '' }}" value="{{null !== $user->address ?  $user->address->city : ''}}" name="city" required>
                                        </div>
                                    </div>
                                        ...
                                        @if(null !== $user->address)
                                            <select name="state" class="wide  form-control input-md custom_input">
                                                @foreach($states as $state)
                                                    @if($state->id == $user->address->state_id)
                                                        <option selected="selected" value="{{ $user->address->state_id }}">{{ $state->name }}</option>
                                                    @else
